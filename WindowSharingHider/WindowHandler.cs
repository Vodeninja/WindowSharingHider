    ...
    public static class WindowHandler
    {
        ...
        public static Dictionary<IntPtr, String> GetVisibleWindows()
        {
            ...
            EnumWindows(delegate (IntPtr hWnd, IntPtr lParam)
            {
                ...
                {
                    var exeNameBuilder = new StringBuilder(1024);
                    GetModuleFileNameEx(processHandle, IntPtr.Zero, exeNameBuilder, (uint)exeNameBuilder.Capacity);
                    var exeFullPath = exeNameBuilder.ToString();
                    var exeName = Path.GetFileName(exeFullPath);

                    windows[hWnd] = $"{exeName} - {windowTitle}";
                }
        ...
        public static void SetWindowDisplayAffinity(IntPtr hWnd, Int32 dwAffinity)
        {
            ...
            if (is32Bit)
            {
                asm.Add(0x68); asm.AddRange(BitConverter.GetBytes((UInt32)dwAffinity));
                asm.Add(0x68); // push
        ...
        public static void RestoreWindowDisplayAffinity(Dictionary<IntPtr, Int32> windowAffinities)
        {
            foreach (var entry in windowAffinities)
            {
                SetWindowDisplayAffinity(entry.Key, entry.Value);
            }
        }

        public static void Main(string[] args)
        {
            var windowAffinities = new Dictionary<IntPtr, Int32>();
            var visibleWindows = GetVisibleWindows();
            foreach (var window in visibleWindows)
            {
                windowAffinities[window.Key] = 0x00000001; // WDA_MONITOR
            }

            RestoreWindowDisplayAffinity(windowAffinities);
